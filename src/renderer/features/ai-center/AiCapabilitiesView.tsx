...
import { ImageGenerationConcurrencyControl } from '@/renderer/features/ai-center/ImageGenerationConcurrencyControl';
...
function modelsForExtension(extension: ExtensionDto, routes: ImageGenerationRouteDto[]) {
  const providerIds = new Set(extension.manifest.contributes.modelProviders ?? []);
  return routes.filter((route) => providerIds.has(resolveImageGenerationRouteExecutionIdentity(route).providerId));
}

export function AiCapabilitiesView({ active, data, locale, notify, refresh, onConfigure, onManagePlugins }: Props) {
  const { messages } = useI18n();
  const l = messages.aiCenter;
  const capabilityExtensions = (data.extensions ?? []).filter(
    (extension) => (extension.manifest.contributes.modelProviders?.length ?? 0) > 0,
  );

  return (
    <ScrollArea type="always" className="h-full min-h-0 bg-background">
                  ...
                  <div className="min-w-0">
                    ...
                    {routes.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {routes.map((route) => (
                          <ImageGenerationConcurrencyControl
                            key={route.key}
                            route={route}
                            refresh={refresh}
                            notify={notify}
