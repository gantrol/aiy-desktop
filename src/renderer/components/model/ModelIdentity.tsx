...
function isOpenAiModel(providerKey: string, modelId: string) {
  return (
    ['codex', 'openai'].includes(providerKey.toLocaleLowerCase()) || /^(?:gpt-|chatgpt-|o[134](?:-|$))/iu.test(modelId)
  );
}
...
export function ModelMark({
  ...
  return (
    <span
      aria-hidden="true"
      className={cn('grid size-5 shrink-0 place-items-center rounded-full bg-foreground text-background', className)}
    >
      {isOpenAiModel(providerKey, modelId) ? <OpenAiMark /> : <CpuIcon className="size-3.5" />}
    </span>
  );
}
...
export function ModelIdentity({
  ...
  return (
    <span className={cn('inline-flex min-w-0 items-center gap-1.5', className)} title={`${providerKey} · ${modelId}`}>
      <ModelMark providerKey={providerKey} modelId={modelId} />
      <span className="truncate text-xs font-medium text-foreground">{modelDisplayName(modelId)}</span>
    </span>
  );
}
