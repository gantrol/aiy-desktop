...
import { ArrowLeftIcon, FileTextIcon, HistoryIcon, ImportIcon, PencilIcon, PlusIcon } from 'lucide-react';
...
import { derivedVisualRoleTitle } from '@/renderer/components/creator/screen/creatorScreenProjection';
...
function versionOption(item: PromptVersionDto, importedPrompt: string, labelPrefix?: string) {
  const summary =
...
function derivedHeader(props: Props, derived: DerivedHeader) {
  return (
    <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden">
      <Button
        ...
        size="icon-sm"
        className="shrink-0"
        title={props.locale === 'zh' ? '返回来源' : 'Back to source'}
        aria-label={props.locale === 'zh' ? '返回来源' : 'Back to source'}
      ...
      <strong className="shrink-0 text-sm">{derivedVisualRoleTitle(derived.visual.role, props.locale)}</strong>
      <Combobox
        ...
        options={derived.schemes.map((visual, index) => {
          ...
          const label = `${props.locale === 'zh' ? '方案' : 'Concept'} ${String(index + 1).padStart(2, '0')}${
            applied ? (props.locale === 'zh' ? ' · 已应用' : ' · Applied') : ''
          }`;
        ...
        ariaLabel={props.locale === 'zh' ? '封面方案' : 'Cover concept'}
        placeholder={`${props.locale === 'zh' ? '方案' : 'Concept'} ${String(derived.schemeIndex + 1).padStart(2, '0')}`}
        searchPlaceholder={props.locale === 'zh' ? '搜索方案' : 'Search concepts'}
        emptyText={props.locale === 'zh' ? '没有匹配方案' : 'No matching concepts'}
        disabled={derived.creating}
        action={{
          label: props.locale === 'zh' ? '新建方案' : 'New concept',
      ...
      />
      {versionSelector(props, 'compact')}
    </div>
  );
}

function versionSelector(props: Props, width: 'compact' | 'wide'): ReactNode {
  if (!props.series?.versions.length) return null;
  const options = props.series.versions.map((item) => {
    if (width === 'compact') return versionOption(item, props.labels.importedPrompt, props.locale);
  ...
  return (
    <Combobox
      ...
      ariaLabel={props.labels.version}
...
function CreatorInputHeaderContent(props: Props) {
  const main = props.derived ? (
    derivedHeader(props, props.derived)
      ...
      <div className="flex items-center gap-1.5">
        {!props.derived && (
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            title={props.locale === 'zh' ? '输入暂存与版本' : 'Input stashes & versions'}
            aria-label={props.locale === 'zh' ? '输入暂存与版本' : 'Input stashes & versions'}
        ...
        {!props.derived && props.creationMode === 'new' && props.creationStartMode === 'image' && (
          <Button type="button" variant="ghost" size="sm" onClick={props.onOpenExternalImport}>
            <ImportIcon className="size-3.5" />
            {props.locale === 'zh' ? '导入外部创作' : 'Import external'}
          </Button>
        )}
      </div>
    </header>
