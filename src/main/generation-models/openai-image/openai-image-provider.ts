...
import { OPENAI_IMAGE_MODELS } from '@/main/extensions/openai-image-api/definition';
...
function supportedGptImageSize(width: number | null, height: number | null) {
...
export class OpenAiImageProvider implements GenerationProvider {
  ...
  constructor(
    ...
    const descriptor = (model: (typeof OPENAI_IMAGE_MODELS)[number]): ImageGenerationRouteDto => {
      ...
      return {
        ...
        executionIdentity: {
          ...
          modelId: model.modelId,
          canonicalModelFamilyId: model.key,
      ...
      (model) =>
        new AdapterBackedGenerationModel({
          ...
          createRequest: ({ runId, input, route }) => {
            ...
            // Imported edit sources may use arbitrary dimensions. Let GPT Image
            ...
            const useAutomaticEditSize = Boolean(sourceAssetId) && !supportedGptImageSize(input.width, input.height);
            return {
              runId,
              modelKey: route.key,
