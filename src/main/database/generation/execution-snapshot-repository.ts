...
export class ExecutionSnapshotRepository {
  ...
  resolveGenerationInput(
    ...
    // Direct repository callers and pre-profile routes retain the historical
    // behavior. Live generation always supplies the route's explicit profile.
    const effectivePromptProfileId =
      promptProfileId?.trim() || imageGenerationPromptProfileId({ key: input.modelKey, modelId: input.modelKey });
    const composition = this.resolveComposition(commonInput, effectivePromptProfileId);
  ...
  freezeGenerationExecution(input: FreezeGenerationExecutionInput) {
    ...
    const commonInput: GenerationExecutionCommonInputDto = {
      ...
      resolvedPrompt: this.resolveComposition(input.promptInput, imageGenerationPromptProfileId(input.route)),
  ...
  private resolveComposition(commonInput: PromptCommonInputDto, promptProfileId: string): ResolvedPromptComposition {
    ...
    return resolvePromptComposition({
      ...
      directTerms: commonInput.directTerms.flatMap(
        (term) =>
          this.termExpression(term, promptProfileId, term.promptLocale ?? commonInput.directTermPromptLocale) ?? [],
      ),
      recipes: commonInput.recipes.map((recipe) => this.recipeExpression(recipe, promptProfileId)),
  ...
  private termExpression(
    ...
    const candidates = (
      this.db
        ...
        revision.title_locale,
        expression.id AS expression_id,
        expression.locale AS expression_locale,
        profile.stable_key AS context_key,
      ...
      JOIN term_expressions expression
        ON expression.term_revision_id = revision.id AND expression.model_key = ?
      ...
      ORDER BY expression.id DESC`,
        )
        .all(locale, modelKey, term.termRevisionId, term.termId) as JsonMap[]
    ).map((row) => ({
      contextKey: text(row.context_key),
      locale: text(row.expression_locale),
      primaryLocale: text(row.title_locale),
      expressionRevisionId: text(row.expression_id),
      label: text(row.label),
      positiveExpression: text(row.positive_expression),
      negativeExpression: text(row.negative_expression),
    }));
    const expression = selectTermExpressionCandidate(candidates, locale, candidates[0]?.primaryLocale ?? locale);
    if (!expression) return null;
    const { positiveExpression, negativeExpression } = expression;
    if (!positiveExpression.trim() && !negativeExpression.trim()) return null;
    return {
      stableId: term.termId,
      revisionId: term.termRevisionId,
      expressionRevisionId: expression.expressionRevisionId,
