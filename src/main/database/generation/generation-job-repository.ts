...
export interface GenerationProviderAcceptance {
  ...
  checkpoint?: unknown;
  /** The identifier belongs to a durable remote operation, not merely a synchronous HTTP trace. */
  remoteOperationAccepted?: boolean;
}
...
export class GenerationJobRepository {
  ...
  createForRun(runId: string, retryOfRunId?: string): GenerationJobState {
    ...
    const retryOf = this.retrySourceJob(retryOfRunId);
    ...
    this.recordInitialOutput(run, jobId, attemptId, createdAt);
    this.appendEvent(
      jobId,
      attemptId,
  ...
  private retrySourceJob(retryOfRunId?: string) {
    if (!retryOfRunId) return undefined;
    const retryOf = this.db
      .prepare(
    ...
    if (!retryOf) throw new Error('Generation retry source job not found');
    return retryOf;
  }

  private recordInitialOutput(run: JsonMap, jobId: string, attemptId: string, createdAt: string) {
  ...
  recordProviderAccepted(runId: string, input: GenerationProviderAcceptance): GenerationJobState {
    ...
    return this.db.transaction(() => {
      ...
      const remoteOperationAccepted = input.remoteOperationAccepted === true;
      ...
      this.db
        ...
        provider_request_id = ?, checkpoint_json = ?, last_heartbeat_at = ?,
        recovery_mode = CASE WHEN ? = 1 OR ? IS NOT NULL THEN 'RECONCILE' ELSE recovery_mode END,
      ...
      this.appendEvent(
        state.jobId,
        state.attemptId,
        remoteOperationAccepted ? 'REMOTE_OPERATION_ACCEPTED' : 'REQUEST_IDENTIFIED',
        state.phase,
        state.progress,
        state.statusMessage,
        { providerKey, providerRequestId, remoteOperationAccepted },
        acceptedAt,
      );
      return this.getForRun(runId)!;
    })();
  ...
  saveCheckpoint(runId: string, checkpoint: unknown, progress?: number | null): GenerationJobState {
    return this.db.transaction(() => {
      ...
      this.db
        ...
        recovery_mode = CASE WHEN ? IS NOT NULL THEN 'RECONCILE' ELSE recovery_mode END,
        last_heartbeat_at = ?, updated_at = ? WHERE id = ?`,
        )
        .run(checkpointJson, nextProgress, checkpointJson, changedAt, changedAt, state.attemptId);
      this.appendEvent(
        state.jobId,
        state.attemptId,
        'CHECKPOINT_SAVED',
  ...
  listRecoverable(): RecoverableGenerationRun[] {
    ...
    return rows.map((row) => {
      ...
      const hasReconciliationEvidence = state.checkpoint !== null || Number(row.has_remote_operation_evidence) === 1;
      return {
        runId: state.runId,
        jobId: state.jobId,
        ...
        recoveryMode: hasReconciliationEvidence ? ('RECONCILE' as const) : ('RETRY' as const),
      };
    });
  }
