...
import { reconcileDictionaryCore, type PreparedDictionarySource } from '@/main/dictionary-core';
...
import { type JsonMap, localized, now, text } from '@/main/database/values';
import { canonicalSnapshotJson, promptInputContentHash, promptInputSourceKind } from '@/main/database/snapshot-content';
import { promptDirectTermsFromBindings, promptRecipesFromBindings } from '@/main/database/prompt-binding-input';
import { readV03FixtureDocument, V03_FIXTURE_SCHEMA_VERSION } from '@/main/database/fixture-contract';
...
function reconcileFixtureRows(storage: LibraryStorage, fixture: JsonMap, demoAssetsRoot?: string) {
  ...
  db.transaction(() => {
    for (const row of list('promptSeries')) {
      ...
      ).run(row.id, title, row.createdAt, row.deletedAt ?? null, title, '');
    }
    for (const row of list('imageAssets')) {
      ...
      const source =
        resolvedAssetsRoot && sourceRelativePath ? path.resolve(resolvedAssetsRoot, sourceRelativePath) : '';
    ...
    for (const row of list('annotations')) {
    ...
    for (const row of list('albums')) {
      const createdAt = text(row.createdAt) || now();
      db.prepare(
        `INSERT OR IGNORE INTO albums
        (id, title, intent, defaults_json, pinned, created_at, updated_at, archived_at, deleted_at)
        VALUES (?, ?, '', '{}', 0, ?, ?, NULL, ?)`,
      ).run(row.id, row.title, createdAt, createdAt, row.deletedAt ?? null);
    }
