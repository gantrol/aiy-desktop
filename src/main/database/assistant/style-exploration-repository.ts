...
import {
  DirectionExperimentTaskRepository,
  type DirectionExperimentRunHydration,
} from '@/main/database/assistant/direction-experiment-task-repository';
...
export class StyleExplorationRepository {
  ...
  validateStart(input: StyleExplorationStartInput, promptSnapshots: readonly PromptCommonInputDto[]): void {
    ...
    this.assertDelegation(input.delegation, result);
    ...
    this.assertFrozenSlots(input, promptSnapshots, {
      termIds: frozenTermIds,
      referenceAssetIds: frozenReferences,
      recipes: frozenRecipes,
      canvasPresetKey: frozenCanvasPresetKey,
      width: frozenCanvasWidth,
      height: frozenCanvasHeight,
      promptContext: frozenPromptContext,
    });
  }

  private assertDelegation(delegation: StyleExplorationStartInput['delegation'], result: JsonMap) {
    if (!delegation) return;
    if (!delegation.objective.trim()) throw new Error('Delegated direction experiment requires an objective');
    if (!delegation.remoteScope.map((value) => value.trim()).filter(Boolean).length) {
      throw new Error('Delegated direction experiment must disclose its remote send scope');
    }
    if (delegation.deadlineAt && !Number.isFinite(Date.parse(delegation.deadlineAt))) {
    ...
    const acceptedDecisions = delegation.decisions.map((decision) => ({
  ...
  private assertFrozenSlots(
    input: StyleExplorationStartInput,
    promptSnapshots: readonly PromptCommonInputDto[],
    frozen: {
      termIds: string[];
    ...
    for (const [index, slot] of input.slots.entries()) {
      if (
        !sameStrings(slot.input.termIds, frozen.termIds) ||
        !sameStrings(slot.input.referenceAssetIds, frozen.referenceAssetIds) ||
        canonicalJson(slot.input.wordPaletteReferences) !== canonicalJson(frozen.recipes) ||
        slot.input.canvasPresetKey !== frozen.canvasPresetKey ||
        slot.input.width !== frozen.width ||
        slot.input.height !== frozen.height
      ) {
      ...
      if (canonicalJson(currentPromptContext) !== canonicalJson(frozen.promptContext)) {
  ...
  get(batchId: string): StyleExplorationBatchDto | null {
    const row = this.db.prepare('SELECT * FROM style_exploration_batches WHERE id = ?').get(batchId) as
      JsonMap | undefined;
    return row ? this.hydrateBatches([row])[0] : null;
  }
  ...
  private hydrateBatches(rows: readonly JsonMap[]): StyleExplorationBatchDto[] {
    if (!rows.length) return [];
    const batchIds = rows.map((row) => text(row.id));
    const slotRows = this.db
      .prepare(
        `SELECT slot.* FROM style_exploration_slots slot
      JOIN json_each(?) selected_batch ON selected_batch.value = slot.batch_id
      ORDER BY slot.batch_id, slot.sort_order, slot.id`,
      )
      .all(JSON.stringify(batchIds)) as JsonMap[];
    const slotsByBatch = new Map<string, JsonMap[]>();
    for (const slot of slotRows) {
      const batchId = text(slot.batch_id);
      const slots = slotsByBatch.get(batchId);
      if (slots) slots.push(slot);
      else slotsByBatch.set(batchId, [slot]);
    }
    const runs = this.directorTasks.loadRunHydration(batchIds);
    const tasks = this.directorTasks.listForBatches(batchIds, runs);
    return rows.map((row) =>
      this.batchDto(row, slotsByBatch.get(text(row.id)) ?? [], runs, tasks.get(text(row.id)) ?? null),
    );
  }

  private batchDto(
    row: JsonMap,
    slotRows: readonly JsonMap[],
    runs: DirectionExperimentRunHydration,
    directorTask: StyleExplorationBatchDto['directorTask'],
  ): StyleExplorationBatchDto {
    const batchId = text(row.id);
    const slots = slotRows.map((slot) =>
      this.slotDto(slot, runs.allBySlot.get(text(slot.id)) ?? [], runs.currentBySlot.get(text(slot.id)) ?? []),
    );
    const projection = statusProjection(runs.currentByBatch.get(batchId) ?? []);
    return {
      id: batchId,
      scope: {
  ...
  private slotDto(
    row: JsonMap,
    hydratedRuns?: readonly RunProjection[],
    hydratedCurrentAttempts?: readonly RunProjection[],
  ): StyleExplorationSlotDto {
    const slotId = text(row.id);
    const runs = hydratedRuns ?? this.allAttemptRuns(slotId);
    const currentAttempts = hydratedCurrentAttempts ?? this.currentAttemptRuns(slotId);
    return {
      id: slotId,
  ...
  private allAttemptRuns(slotId: string): RunProjection[] {
    const rows = this.db
      .prepare(
