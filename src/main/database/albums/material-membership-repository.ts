...
export class MaterialMembershipRepository {
  ...
  createAlbumFromMaterials(input: AlbumCreateFromMaterialsInput): AlbumCreateFromMaterialsResult {
    ...
    return this.db
      .transaction(() => {
        const created = this.albums.create({ title: input.title, titleLocale });
        const result = this.addToDestinations({
          targets: input.targets,
          albumIds: [created.id],
          termIds: [],
        });
        const album = this.albums.list(titleLocale).find((candidate) => candidate.id === created.id);
    ...
    if (materialIds.length > 0) {
      ...
      for (const row of rows) {
        ...
        byMaterialId.set(text(row.id), {
          ...
          kind,
          imageAssetId: kind !== 'TEXT' ? text(row.image_asset_id) : null,
        });
      }
