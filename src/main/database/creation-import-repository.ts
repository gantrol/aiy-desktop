...
import { rehomeIdeaCreation } from '@/main/database/idea-creation-lifecycle';
...
} from '@/main/database/imported-output-linker';
...
export class CreationImportRepository {
  ...
  updateOutput(input: ImportedCreationOutputUpdateInput) {
    const displayName = input.displayName.trim();
    if (!displayName) throw new Error('Name is required');
    const output = this.db
      .prepare(
        `SELECT series_id, display_name FROM creation_output_imports
      WHERE id = ? AND deleted_at IS NULL`,
      )
    ...
    const modelKey = comparisonRole === 'MODEL' ? input.modelKey : null;
    ...
    const hasDeclaredGenerationData =
    ...
    const result = this.db
      ...
      .run(
        ...
        input.generationText.trim(),
        hasDeclaredGenerationData ? 'DECLARED' : 'UNKNOWN',
        comparisonRole,
        input.outputId,
      );
  ...
  private resolveSeries(context: CreatorImageImportContext, firstOriginalName?: string) {
    ...
    this.db
      ...
      .run(seriesId, title, now(), title, '');
    this.storage.recordChange('PROMPT_SERIES', seriesId, 'CREATE', { title, source: context.source });
    return seriesId;
  }
  ...
  private availableSeriesTitle(base: string) {
    const used = new Set(
      (
        this.db
          .prepare(
            `SELECT title, title_zh, title_en FROM prompt_series
