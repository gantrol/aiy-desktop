...
import type { PackRepository, PackReleaseItemInput } from '@/main/database/packs/pack-repository';
import type { FixturePackProfile } from '@/main/database/packs/fixture-pack-profile';
import {
  fixtureContentHash,
  fixturePaletteCatalogContentHash,
  ...
  isCurrent(source: FixturePackSource) {
    ...
    return this.storage.db.transaction(() => {
      ...
      for (const term of source.terms) {
        const local = this.storage.db.prepare('SELECT id FROM terms WHERE stable_key = ?').get(term.stableKey) as
          JsonMap | undefined;
        if (!local) throw new Error(`Content package term was not materialized: ${term.stableKey}`);
        const localObjectId = text(local.id);
        localTermIds.set(term.stableKey, localObjectId);
      ...
      for (const recipe of source.recipes) {
        const localContentHash = fixturePaletteCatalogContentHash(recipe.source, localTermIds);
        const local = this.storage.db
          ...
          .get(recipe.localObjectId, localContentHash) as JsonMap | undefined;
