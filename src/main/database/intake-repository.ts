...
import type { CreationImportRepository } from '@/main/database/creation-import-repository';
import {
  defaultImportedImageMetadata,
  writeImportedMaterialMetadata,
} from '@/main/database/external-material-import-metadata';
...
export class IntakeRepository {
    ...
    return this.db
      .transaction(() => {
        ...
        for (const item of input.items) {
          ...
          const staged = stagedImages.get(item.id);
          if (!staged) throw new Error(`Image could not be staged: ${item.name}`);
          const imported = this.ensureImageMaterial(
            staged,
            input.source,
            item.name,
            item.sourceUrl ?? '',
          ...
          seenMaterialIds.add(materialId);
          materialIds.push(materialId);
          imageMaterialIds.push(materialId);
    ...
    return this.db
      .transaction(() => {
        ...
        if (existing) {
          this.db
            .prepare(
              `UPDATE creation_drafts SET target_album_id = ?, text_content = ?, title_zh = ?, title_en = ?,
          ...
          this.db
            .prepare(
              `INSERT INTO creation_drafts
          (id, text_content, created_at, updated_at, consumed_at, source_series_id, deleted_at,
            title_zh, title_en, term_prompt_locale, term_ids_json, palette_references_json, prompt_nodes_json,
  ...
  private ensureImageMaterial(
    ...
    const existingAsset = this.db
      ...
      WHERE object_hash = ? AND deleted_at IS NULL ORDER BY created_at LIMIT 1`,
      )
      .get(staged.stored.hash) as JsonMap | undefined;
    ...
    if (!existingAsset) {
      ...
      this.storage.recordChange('IMAGE_ASSET', assetId, 'IMPORT', { source });
    }
    ...
    if (!existingMaterial) {
      this.db
        ...
        .run(materialId, assetId, staged.stored.hash, source, now());
      this.storage.recordChange('MATERIAL', materialId, 'CREATE', { kind: 'IMAGE', assetId, source });
    }
  ...
  private getDraft(draftId: string): CreationDraftDto {
    ...
    const title = text(draft.title_zh);
    return {
      id: draftId,
