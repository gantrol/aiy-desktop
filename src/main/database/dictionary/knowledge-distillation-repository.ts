...
import { normalizeSearchText, searchTokens } from '@/main/database/dictionary/search-tokenizer';
  ...
  private source(assetId: string, locale: Locale): FrozenSource {
    ...
    SELECT * FROM (
      SELECT 'GENERATION_RUN' AS source_kind, run.id AS source_id, run.model_key,
        ...
        series.title, series.title_locale, version.id AS prompt_version_id,
