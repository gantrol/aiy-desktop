...
import type { StoredCreatorImage } from '@/main/database/creation-import-repository';
import type { LibraryStorage } from '@/main/database/storage';
import { now, type JsonMap, text } from '@/main/database/values';
      ...
      .transaction(() => {
        ...
        for (const [index, output] of result.importedOutputs.entries()) {
          ...
          this.creationImports.updateOutput({
            ...
            note: `Codex task ${codexTask.threadId}`,
            sourceUrl: '',
            aiGeneratedStatus: 'YES',
            comparisonRole: 'MODEL',
            modelKey: null,
            modelName: 'Codex ImageGen',
            modelProvider: 'Codex',
            modelVersion: '',
