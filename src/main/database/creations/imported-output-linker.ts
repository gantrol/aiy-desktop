...
export function linkExistingImportedOutput(
  ...
  if (existing) {
    // Linking is an import operation, not an edit operation. Reusing the same
    // asset in the same creation must not replace the existing output's source
    // facts with metadata from a later import review.
