...
import { readV03FixtureDocument } from '@/main/database/fixture-contract';
...
function paletteCatalogContentHash(palette: JsonMap, termIds: Map<string, string>) {
...
export function readFixturePackSource(profile: FixturePackProfile, paths: FixturePackSourcePaths): FixturePackSource {
  ...
  const termRows = [...inlineTerms, ...catalogTerms];
  ...
  const recipes = maps(palette.palettes).map((recipe): FixturePackSourceRecipe => {
    ...
    return {
      ...
      localContentHash: paletteCatalogContentHash(recipe, termIds),
