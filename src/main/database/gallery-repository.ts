...
import { isSystemMaterialAlbumId, materialAlbumAssetFilter } from '@/main/database/material-album-repository';
...
const gallerySearchPredicate = `(
  ...
  OR EXISTS (
    ...
    WHERE search_material.image_asset_id = asset.id
      AND search_material.kind = 'IMAGE' AND search_material.deleted_at IS NULL
      AND (
  ...
  OR EXISTS (
    ...
    WHERE search_series.deleted_at IS NULL
      AND (
        ...
        OR LOWER(COALESCE(search_series.title_zh, '')) LIKE ? ESCAPE '\\'
        OR LOWER(COALESCE(search_series.title_en, '')) LIKE ? ESCAPE '\\'
      )
      AND (
        EXISTS (
...
const creationRelationshipsCte = `creation_candidates AS (
  SELECT run.result_asset_id AS asset_id, run.id AS run_id, NULL AS imported_output_id,
    ...
    version.version_no, series.id AS series_id, series.title, series.title_zh, series.title_en,
...
creation AS (
  SELECT ranked.asset_id, ranked.run_id, ranked.imported_output_id,
    ...
    ranked.title, ranked.title_zh, ranked.title_en,
...
export class GalleryRepository {
  ...
  list(input: GalleryListInput): GalleryPageDto {
    ...
    const searchParameters = query ? new Array<string>(9).fill(searchPattern) : [];
    ...
    const rows = this.db
      ...
      favorite_ranked AS (
        ...
        WHERE favorite.deleted_at IS NULL AND material.deleted_at IS NULL
          AND material.kind = 'IMAGE' AND material.image_asset_id IS NOT NULL
      ),
      favorite AS (
      ...
      SELECT asset.id, asset.kind AS asset_kind, asset.origin_type, asset.width, asset.height,
        ...
        creation.title AS series_title, creation.title_zh AS series_title_zh,
        creation.title_en AS series_title_en, dictionary.term_id,
      ...
      LEFT JOIN materials gallery_material ON gallery_material.image_asset_id = asset.id
        AND gallery_material.kind = 'IMAGE' AND gallery_material.deleted_at IS NULL
    ...
    const failedOutputCountVisibilityClause = `AND (
      ...
      OR EXISTS (
        ...
        WHERE visible_material.image_asset_id = asset.id
          AND visible_material.kind = 'IMAGE'
    ...
    const items = pageRows.map((row) => this.toItem(row, input));
  ...
  private countPredicate(source: GallerySourceFilter) {
    ...
    const dictionary = `EXISTS (
      SELECT 1 FROM (${dictionaryAssetTerms}) selected_dictionary
      JOIN terms term ON term.id = selected_dictionary.term_id
      WHERE selected_dictionary.asset_id = asset.id AND term.archived_at IS NULL
    )`;
    const favorite = `EXISTS (
      ...
      WHERE material.image_asset_id = asset.id AND material.kind = 'IMAGE'
    ...
    const material = `EXISTS (
      ...
      WHERE selected_material.image_asset_id = asset.id AND selected_material.kind = 'IMAGE'
  ...
  private toItem(row: JsonMap, _input: GalleryListInput): GalleryItemDto {
    ...
    const rating = (dimension: ImageRatingDto['dimension'], prefix: 'aesthetic' | 'realism'): ImageRatingDto | null =>
      row[`${prefix}_rating_id`]
        ...
        : null;
    const seriesTitle = text(row.series_title);
    ...
    return {
      ...
      metadata: row.metadata_updated_at
        ? {
            ...
            modelKey: row.metadata_model_key ? text(row.metadata_model_key) : null,
