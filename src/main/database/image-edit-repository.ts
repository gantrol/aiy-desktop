...
import { IMAGE_EDIT_MASK_RASTERIZER_VERSION, rasterizeImageEditMask } from '@/main/image-edit/mask-rasterizer';
import type { LibraryStorage } from '@/main/database/storage';
import { annotationDto, type JsonMap, now, text } from '@/main/database/values';
...
export class ImageEditRepository {
  ...
  persistForRun(
    ...
    // Freeze a deterministic mask for every targeted edit. Native-mask
    // providers map it to their mask field; semantic providers can still use
    // the same artifact as an attached visual localization guide.
