...
export class AssistantRunRepository {
  ...
  get(id: string): AssistantRunDto | null {
    ...
    return row ? this.hydrate(toRun(row)) : null;
  }
  ...
  list(scope: CreatorAgentScope): AssistantRunDto[] {
    ...
    return rows.map((row) => this.hydrate(toRun(row)));
  }
  ...
  private hydrate(run: AssistantRunDto): AssistantRunDto {
    const creation = this.creations.getForAssistantRun(run.id);
    return {
      ...run,
      creationId: creation?.id ?? null,
      creationTitle: creation?.title ?? null,
      activityEvents: creation
        ? creation.activityEvents.filter((event) => event.assistantRunId === null || event.assistantRunId === run.id)
        : [],
    };
  }
