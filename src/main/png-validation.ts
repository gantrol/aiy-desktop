...
function* pngValidationSteps(bytes: Buffer): Generator<PngCrcStep, ValidPngStructure | null, number> {
  ...
  while (offset < bytes.length) {
    ...
    const chunkCrc = yield { start: typeOffset, end: dataEnd };
    if (chunkCrc !== bytes.readUInt32BE(dataEnd)) return null;
    ...
    if (type === 'IHDR') {
      ...
      if (sawPalette || sawImageData || colorType === 0 || colorType === 4) return null;
      if (dataLength === 0 || dataLength > 768 || dataLength % 3 !== 0) return null;
      sawPalette = true;
    } else if (type === 'IDAT') {
      ...
      if (dataLength !== 0 || !sawImageData || imageDataBytes === 0 || chunkEnd !== bytes.length) return null;
      return { width, height };
