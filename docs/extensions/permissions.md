...
浏览器扩展只接收目标为 `weibo` 的一次性交接。浏览器端的 `nativeMessaging` 和精确微博 host permission 不替代桌面端的 `browser.handoff:weibo`。当前能力只填入草稿；最终发布必须由用户操作。未来若增加实际发布，必须使用独立的 `platform.publish:weibo` 高风险能力，并对每篇内容重新确认。
...
- `<all_urls>`、任意 HTTP 网络或未规范化的自定义端点；
- 由持久授权触发的静默发布、删除或覆盖；
- 从远端下载并执行扩展代码。
...
7. **浏览器复核**：浏览器伴侣只声明 ChatGPT 与微博的精确 host，使用 Native Messaging；领取内容时同时校验发送方 tab、当前站点和交接目标。URL 只携带随机交接 ID，媒体按长度和 SHA-256 复核，适配器不寻找发布控件。
